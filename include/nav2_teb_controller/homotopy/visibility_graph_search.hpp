...
{
public:
  explicit VisibilityGraphSearch(double inflation_radius = 0.2,
                                 double robot_radius = 0.3);

  bool search(const PoseSE2& start,
              const PoseSE2& goal,
...
private:
   ...
   */
  std::vector<int> dijkstra(const VisibilityGraph& graph,
                             int start_id, int goal_id) const;

  /**
   * @brief Yen's K-Shortest Paths for alternative homotopy classes
   */
  std::vector<std::vector<int>> yenKShortestPaths(
   ...
   */
  std::vector<PoseSE2> nodePathToPoses(
    const VisibilityGraph& graph,
