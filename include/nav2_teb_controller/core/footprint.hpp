...
public:
  ...
  Footprint(bool use_local_costmap,
            const std::string& model,
            const std::string& points)
  : use_local_costmap_(use_local_costmap)
  {
    if      (model == "circles") model_ = Model::Circles;
    else if (model == "polygon") model_ = Model::Polygon;
    else throw std::invalid_argument(
        "[Footprint] Unknown model '" + model + "'. Use 'circles' or 'polygon'.");
  ...
  [[nodiscard]] FootprintCheckResult check(double x, double y, double theta,
                                            const ObstacleMap2D& esdf,
                                            double d_min = 0.0) const
  {
    ...
    if (model_ == Model::Circles) {
      if (vals.size() % 3 != 0)
        throw std::invalid_argument(
            "[Footprint] model=circles: need N×3 values (cx,cy,r), got "
      ...
      if (vals.size() % 2 != 0)
        throw std::invalid_argument(
            "[Footprint] model=polygon: need N×2 values (x,y), got "
            + std::to_string(vals.size()));
