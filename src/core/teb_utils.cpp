  ...
  int min_samples, bool guess_backwards_motion, bool /*fixed_goal*/)
{
  ...
  for (int i = 1; i < static_cast<int>(path.poses.size()) - 1; ++i) {
    ...
    PoseSE2 intermediate_pose(path.poses[i].pose.position.x,
                               path.poses[i].pose.position.y, yaw);
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("optimal_planner"), 
        "TEB Utils: Added path poses to teb.");
  ...
  bool fast_mode)
{
  if (teb.sizeTimeDiffs() != 0 && teb.sizeTimeDiffs() + 1 != teb.sizePoses())
  {
    RCLCPP_ERROR(rclcpp::get_logger("optimal_planner"),
      "TEB Utils: Auto resize not possible.");
  ...
  for (int rep = 0; rep < 100 && modified; ++rep)
  {
    ...
    for (std::size_t i = 0; i < teb.sizeTimeDiffs(); ++i)
    {
      ...
      const double angle_diff = std::abs(
        angles::normalize_angle(teb.pose(i+1).theta() - teb.pose(i).theta()));
      ...
      const bool angle_significant = angle_diff > max_angle_diff; //0.01;  // ~0.6° — Kurven-Posen schützen
...
{ 
  ...
  if (teb.sizePoses()<2)
    return cmd_vel;
  look_ahead_poses = std::max(1, std::min(look_ahead_poses, static_cast<int>(teb.sizePoses() - 1)));
  double dt = 0.0;
  ...
  // else
    cmd_vel = extractVelocity(teb.pose(0), teb.pose(static_cast<std::size_t>(look_ahead_poses)), dt, holonomic);
  return cmd_vel;
}
...
{
  if (global_plan.poses.empty()) return true;
  try {
    ...
    while (it != global_plan.poses.end()) {
      ...
      if (dx*dx + dy*dy < dist_thresh_sq) { erase_end = it; break; }
      ++it;
    }
    if (erase_end == global_plan.poses.end()) return false;
  ...
  double max_plan_length)
{
  ...
  if (global_plan.poses.empty()) return transformed_path;

  const auto& plan_pose = global_plan.poses.front();
  try {
    auto plan_to_global = tf_buffer.lookupTransform(
      global_frame, tf2_ros::fromMsg(now),
           ...
           && (max_plan_length <= 0.0 || plan_length <= max_plan_length))
    {
      ...
      if (i > 0 && max_plan_length > 0.0)
        plan_length += nav2_util::geometry_utils::euclidean_distance(
          global_plan.poses[i-1].pose, global_plan.poses[i].pose);
      ++i;
    }
...
double computeCurvature(const PoseSE2& p1, const PoseSE2& p2, const PoseSE2& p3)
{
  ...
  if (chord_length < 1e-3) return 0.0;  // Degenerate case
...
int checkFeasibility(const TimedElasticBand& teb, const ObstacleMap2D& esdf, const Footprint& fp, double lookahead)
{
  ...
  for (size_t i = 0; i < teb.sizePoses(); i++)
  {
    ...
    if (progress > lookahead) return -1;
